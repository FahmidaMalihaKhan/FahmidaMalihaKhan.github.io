...
function preload() {
    ...
    playSong = loadSound("sounds/player.m4a")
...
function start() {
    ...
    fill(255)
    textAlign(CENTER)
    text("BLOCK BUSTER!", VIRTUAL_WIDTH / 2, VIRTUAL_HEIGHT / 2)
    textSize(14)
...
function highScores() {
    ...
    textAlign(CENTER)
    textSize(8)
    text("Press esc(escape) to return to the main menu", VIRTUAL_WIDTH / 2, VIRTUAL_HEIGHT - 10)
}
...
function enterHighScore() {
    textSize(14)
    textAlign(CENTER)
    text("AWSOME! Your score: " + score, VIRTUAL_WIDTH / 2, 30)
    ...
    textSize(8)
    text("Use arrow keys to set name initials. Press enter to confirm", VIRTUAL_WIDTH / 2, VIRTUAL_HEIGHT - 10)
}
...
function play() {
    ...
    for (let gbrick of gbricks) {
        if (gbrick.inPlay && ball.collides(gbrick)) {
            ...
            // trigger the bricks hit function
            gbrick.hit()
